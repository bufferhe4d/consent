...
{
 \idm((x, y), G, G')  \<\< \client(Y, G)  \<\< \signserv(\dict)\pclb
 \pcintertext[dotted]{Enroll} \\
  \<\< \text{pick } \login\<\< \\
  \<\< \text{sample } \pseudo \sample \mathcal{D}\<\< \\
  \<\< s \sample \bin^\secpar \<\< \\
...
\pcintertext[dotted]{Consent} \\
  ...
  d \gets \ZZ^*_q \< \<  \<\< \\
  R' \gets d \cdot G\< \<  \<\< \\
  R \gets d \cdot(x \cdot G + Q +  s_\idm \cdot G') \< \<  \<\< \\
...
\begin{pchstack}[center, space=0.2cm, boxed]
    ...
    \procedure[linenumbering]{$\adversary{A}$}{
        \textsf{pick } \login^*, \login\\
        \agentsecret^* \gets \mathsf{OEnroll}(\login^*, \pseudo^*)\\
        \agentsecret \sample \ZZ_q\\
        \mathsf{OCorruptEnroll}(\login, \agentsecret^{-1})\\
        r \sample \ZZ_q\\
        att^* \sample \ZZ_q\\
        \query \gets r\cdot H(\pseudo^*, att^*)\\
        \response \gets \mathsf{OCorruptCommit}(\login, \query)\\
        \consent^* \gets r^{-1}\cdot\agentsecret\cdot\response\\
        \pcreturn \login^*, \pseudo^*, att^*, \consent^*
    }
    \end{pcvstack}
    \end{pchstack}
\caption{\adversary{A} attacking the $\cuf_\signserv$ game.}
